...
def train(train_loader, model, FLAGS, NUM_FILTERS, FILTER_LENGTH1, FILTER_LENGTH2,lamda):
    ...
    with tqdm(train_loader) as t:
        for drug_SMILES, target_protein, affinity in t:
            ...
            # loss_affinity_MAE = loss_func_MAE(pre_affinity, affinity)
            ...
            # loss_MAE = loss_affinity_MAE + 10**lamda * (loss_drug + FLAGS.max_smi_len / FLAGS.max_seq_len * loss_target)
            loss.backward()
            # loss_MAE.backward()
            optimizer.step()
            mse = loss_affinity.item()
            # mae = loss_affinity.item()
            # t.set_postfix(train_loss=loss.item(), mse=mse, train_loss_MAE=loss_MAE.item(), mae=mae, train_cindex=c_index)
...
def test(model,test_loader,FLAGS, NUM_FILTERS, FILTER_LENGTH1, FILTER_LENGTH2, lamda):
    ...
    with torch.no_grad():
        ...
        loss_MAE = loss_func_MAE(torch.Tensor(pre_affinities), torch.Tensor(affinities))
        cindex = get_cindex(affinities,pre_affinities)
        rm2 = get_rm2(affinities, pre_affinities)
        # print("-------------------------------------np.int32(affinities > 7)-----------------------------")
        # print(np.int32(affinities>10))
        # print("-------------------------------------pre_affinities-----------------------------")
        # print(pre_affinities)
        #auc = roc_auc_score(np.int32(affinities > 7), pre_affinities)
        # DAVIS _ DATASET
        # auc = roc_auc_score(np.int32(affinities > 10), pre_affinities)
        auc = roc_auc_score(np.int32(affinities > 12.1), pre_affinities)
        #KIBA _ DATASET
    return cindex, loss, loss_MAE, rm2, auc
...
# ######################################################    Train     #########################################################################
    bestparamind, best_param_list, bestperf, all_predictions, all_losses = general_nfold_cv(XD, XT, Y, label_row_inds,
                                                                                            ...
                                                                                            get_rm2)
# ######################################################    Test     #########################################################################
    best_param, bestperf, all_predictions, all_losses, all_losses_MAE, all_auc, all_aupr = general_nfold_cv_test(XD, XT, Y, label_row_inds, label_col_inds,
    ...
    for test_foldind in range(len(test_sets)):
        ...
        foldloss_MAE = all_losses_MAE[test_foldind]
        testperfs.append(foldperf)
        testloss.append(foldloss)
        testloss_MAE.append(foldloss_MAE)
    ...
    avgloss_MAE = np.mean(testloss_MAE)
    perf_std = np.std(testperfs)
    loss_std = np.std(testloss)
    lossMAE_std = np.std(testloss_MAE)
    ...
    return avgperf, avgloss, avgloss_MAE, perf_std, loss_std, lossMAE_std, avg_auc, auc_std, avg_aupr, aupr_std
...
def general_nfold_cv(XD, XT, Y, label_row_inds, label_col_inds, prfmeasure, FLAGS, labeled_sets, val_sets, get_aupr,
    ...
    logging("----------------Parameter Search------------------train", FLAGS)

    w = len(val_sets)
    print("-------paramset1-------paramset2---------paramset3--------")
    print(paramset1)
    print(paramset2)
    print(paramset3)
    h = len(paramset1) * len(paramset2) * len(paramset3)*len(lamda_set)
    print("------------------------------------------------len(paramset1) * len(paramset2) * len(paramset3)*len(lamda_set)---------------------------------Test------------------------")
    print(h)
...
# ####################################################  Test  ###################################################
                            rperf, loss,loss_MAE, rm2, auc = test(model, test_loader, FLAGS, param1value, param2value, param3value, lamda)

                            rperf_list.append(rperf)
                            ##Set the conditions for early stopping
                            if (epochind+1)%5==0:
                                print('val: epoch:{},p1:{},p2:{},p3:{},loss_MSE:{:.5f},loss_MAE:{:.5f},rperf:{:.5f}, rm2:{:.5f}'.format(epochind, param1value, param2value,
                                                               param3value, loss, loss_MAE, rperf, rm2))
                        ...
                        torch.save(model, 'checkpoint.pth')
                        # break
                        logging("P1 = %d,  P2 = %d, P3 = %d, Fold = %d, CI-i = %f, MSE = %f, MAE = %f,rm2 = %f" % (param1value, param2value, param3value, foldind, rperf, loss,loss_MAE, rm2), FLAGS)
...
def general_nfold_cv_test(XD, XT, Y, label_row_inds, label_col_inds, prfmeasure, FLAGS, labeled_sets, val_sets, get_rm2,
    ...
    logging("---Parameter Search----------------test", FLAGS)
    ...
    all_losses_MAE = [0 for x in range(w)]
    ...
    for foldind in range(len(val_sets)):
        ...
        for epochind in range(FLAGS.num_epoch):
            ...
            if (epochind + 1) % 2 == 0:
                ...
                rperf_list.append(rperf)
                print(
                    'test: epoch:{},p1:{},p2:{},p3:{},loss_MSE:{:.5f},loss_MAE:{:.5f}, rperf:{:.5f}, rm2:{:.5f}'.format(epochind, param1value,
                                                                                                  param2value,
                                                                                                  param3value, loss, loss_MAE,
                                                                                                  rperf, rm2))
        ...
        for drug_SMILES, target_protein, affinity in test_loader:
            pre_affinity, _, _, _, _, _, _, _, _ = model(drug_SMILES, target_protein, FLAGS, param1value, param2value, param3value)
            pre_affinities += pre_affinity.cpu().detach().numpy().tolist()
            affinities += affinity.cpu().detach().numpy().tolist()
        ...
        loss = loss_func(torch.Tensor(pre_affinities), torch.Tensor(affinities))
        loss_MAE = loss_func_MAE(torch.Tensor(pre_affinities), torch.Tensor(affinities))
        print('best: p1:{},p2:{},p3:{},loss_MSE:{:.5f},loss_MAE:{:.5f},rperf:{:.5f}, rm2:{:.5f}'.format(param1value, param2value,
                                                                                              param3value, loss,loss_MAE, rperf,
                                                                                              rm2))

        logging("best: P1 = %d,  P2 = %d, P3 = %d, Fold = %d, CI-i = %f, MSE = %f,MAE = %f, rm2 = %f, auc = %f, aupr = %f" % (
                    param1value, param2value, param3value, foldind, rperf, loss, loss_MAE, rm2, auc, aupr), FLAGS)

        all_predictions[foldind] = rperf  # TODO FOR EACH VAL SET allpredictions[pointer][foldind]
        all_losses[foldind] = loss
        all_losses_MAE[foldind] = loss_MAE
        all_auc[foldind] = auc
    ...
    return best_param_list, best_perf, all_predictions, all_losses, all_losses_MAE, all_auc, all_aupr
...
# ###############################################      Train      ####################################################
        avgperf, avgloss, avgloss_MAE, teststd, lossstd, lossMAE_std, avg_auc, auc_std, avg_aupr, aupr_std = nfold_setting_sample(XD, XT, Y, label_row_inds, label_col_inds, get_cindex, FLAGS, dataset, nfolds,i)

# ###############################################Log  Setting  Type####################################################

        logging("Setting " + str(FLAGS.problem_type), FLAGS)

        logging("avg_perf = %.5f,  avg_mse = %.5f, avg_mae = %.5f, std = %.5f, loss_std = %.5f, lossMAE_std = %.5f, auc = %.5f, auc_std = %.5f, aupr =%.5f, aupr_std = %.5f" %
                    (avgperf, avgloss, avgloss_MAE, teststd, lossstd, lossMAE_std, avg_auc, auc_std, avg_aupr, aupr_std), FLAGS)

        perf.append(avgperf)
        mseloss.append(avgloss)
        maeloss.append(avgloss_MAE)
        auc.append(avg_auc)
    ...
    logging("avg_perf = %.5f,  avg_mse = %.5f, avg_mae = %.5f, std = %.5f, loss_std = %.5f,  lossMAE_std = %.5f, auc = %.5f, auc_std = %.5f, aupr =%.5f, aupr_std = %.5f" %
            (np.mean(perf), np.mean(mseloss), np.mean(maeloss), np.std(perf), np.std(mseloss), np.std(maeloss), np.mean(auc), np.std(auc), np.mean(aupr), np.std(aupr)), FLAGS)
